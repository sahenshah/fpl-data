...
// .wrangler/tmp/pages-1IvJ7d/functionsWorker-0.07522377901139743.mjs
...
__name2(toNumberOrNull, "toNumberOrNull");
async function etagOf(value) {
  const encoded = new TextEncoder().encode(JSON.stringify(value));
  const digest = await crypto.subtle.digest("SHA-256", encoded);
  return Array.from(new Uint8Array(digest)).map((byte) => byte.toString(16).padStart(2, "0")).join("");
}
__name(etagOf, "etagOf");
__name2(etagOf, "etagOf");
async function cachedJsonResponse(value, cacheControl, ifNoneMatch) {
...
__name2(cachedJsonResponse, "cachedJsonResponse");
var FPL_BASE_URL = "https://fantasy.premierleague.com/api";
...
var onRequestGet2 = /* @__PURE__ */ __name2(async ({ env: env2, params, request }) => {
  ...
  `;
  const gameweek = currentEventRows[0]?.id ?? nextEventRows[0]?.id ?? null;
  if (gameweek === null) {
...
var onRequestGet3 = /* @__PURE__ */ __name2(async ({ env: env2, params }) => {
  const sql = getSql(env2);
  ...
  `;
  return new Response(JSON.stringify({ history: rows.map((row) => row.raw_data) }), {
    headers: { "Content-Type": "application/json" }
  });
}, "onRequestGet");
var onRequestGet4 = /* @__PURE__ */ __name2(async ({ env: env2, request }) => {
  const sql = getSql(env2);
...
var onRequestGet5 = /* @__PURE__ */ __name2(async ({ env: env2, request }) => {
  const sql = getSql(env2);
  ...
  const currentEventId = currentEventRows[0]?.id ?? null;
  let liveElements = [];
  if (currentEventId !== null) {
    const liveRows = await sql`
      SELECT player_id, raw_data FROM player_gameweek_stats WHERE gameweek = ${currentEventId}
    `;
    liveElements = liveRows.map((row) => ({ id: row.player_id, stats: row.raw_data }));
  }
  ...
  const value = {
    ...
    fixtures: fixtureRows.map((row) => row.raw_data),
    live: { elements: liveElements, event: currentEventId },
    predictions,
    prediction_run: run ? { id: run.id, model_name: run.model_name, model_version: run.model_version, metrics: run.metrics } : null
  };
...
// .wrangler/tmp/bundle-1GpPg5/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1GpPg5/middleware-loader.entry.ts
...
@neondatabase/serverless/index.mjs:
  ...
  *)
*/
//# sourceMappingURL=functionsWorker-0.07522377901139743.js.map
